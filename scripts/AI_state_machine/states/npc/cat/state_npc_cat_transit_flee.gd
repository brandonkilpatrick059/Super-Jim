...
func physics_process(_delta: float) -> void:
		...
		if(!nav_target_reached):
			...
			if (player_ref.global_position.distance_to(get_host_position()) < 256):
				ai_state_machine.transition_to("transit_flee")
			else:
...
func find_furthest_point_from_player(points : Array[Vector2]) -> Vector2:
	var player_ref = get_tree().get_first_node_in_group("player")
...
func enter(_msg := {}) -> void:
	...
	var nav_target : Vector2 = find_furthest_point_from_player(points)
